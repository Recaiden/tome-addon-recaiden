...
function _M:combatSpellpower(mod, add)
   if self:knowTalent(self.T_DARKSIDE_POWER) then
      add = add + self:callTalent(self.T_DARKSIDE_POWER,"getSpellpower") * self:getWil() / 100
   end
...
function _M:combatMindpower(mod, add)
   if self:knowTalent(self.T_DARKSIDE_POWER) then
      add = add + self:callTalent(self.T_DARKSIDE_POWER,"getMindpower") * self:getMag() / 100
   end
