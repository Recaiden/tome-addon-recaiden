...
newTalent{
	...
	info = function(self, t)
		...
		return ([[Teleports within a range of up to %d.  You arrive right next to a targeted creature, or randomly within %d grids of a targeted space.
If Walking Blasphemy is sustained, all enemies will lose track of you.]]):tformat(range, radius)
	end,
}

newTalent{
	name = "Unlife Unequaled", short_name="REK_DREAD_STEP",
	...
	getShield = function(self, t) return 5*self:getCon() + self:combatTalentScale(t, 120, 400) end,
	action = function(self, t)
		local tids = {}
		for tid, _ in pairs(self.talents_cd) do
			local tt = self:getTalentFromId(tid)
			if not tt.fixed_cooldown then
				if tt.type[1]:find("^undead/") and (not tt.type[1]:find("/dreadlord") and not tt.type[1]:find("/dreadmaster")) then
		...
		self:setEffect(self.EFF_DAMAGE_SHIELD, t.getDuration(self, t), {color={0x6b/255, 0x0b/255, 0x0b/255}, power=t.getShield(self, t), shield_intensity=0.3})
		game:playSoundNear(self, "talents/warp")

		return true
	end,
	info = function(self, t)
		return ([[Call on overwhelming necromantic energies to fortify yourself.  This resets the cooldown of your undead racial talents (%s), allows you to walk through walls for the next %d turns, and shields you for %d for the same time.
The shield increases with your Constitution.]]):tformat(self.descriptor.subrace_dread_original or "???", t.getDuration(self, t), t.getShield(self, t))
	end,
}
...
newTalent{
	...
	action = function(self, t)
		...
		while count < t.getMaxSummons(self, t) do
	...
	info = function(self, t)
		return ([[Summon Dreads until you have %d.  Each Dread is level %d, and annoyingly blinks around, attacking your foes.  Dreads last until destroyed.
You always know where your Dreads are.]]):tformat(t.getMaxSummons(self, t), math.max(1, self.level + t.getLevel(self, t)))
	end,
}
