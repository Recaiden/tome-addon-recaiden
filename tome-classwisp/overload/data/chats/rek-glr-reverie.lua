...
local function regenerateChat()
	...
	for tid, _ in pairs(src.talents_cd) do
		...
		if t and not t.is_inscription and not t.generic and not t.uber and not (t.mode and t.mode == "passive") and not t.fixed_cooldown then
			effcount = effcount+1
		end
	end
		...
		if t and not t.is_inscription and not t.generic and not t.uber and not (t.mode and t.mode == "passive") and not t.fixed_cooldown then
			chat.answers[#chat.answers+1] = {
				...
				action=function()
					...
					if state.remaining <= 0 or effcount <= 1 then coroutine.resume(co, state.recharged) end
				end,
				jump=state.remaining > 1 and effcount > 1 and "welcome" or nil
			}
		end
	end
