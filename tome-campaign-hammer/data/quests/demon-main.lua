...
desc = function(self, who)
	local desc = {}
	desc[#desc+1] = "The peoples of Maj'Eyal are divided, but all of them must fall."
	if self:isCompleted("dwarves") then
		desc[#desc+1] = _t"#LIGHT_GREEN#* It turns out, dwarves aren't Eyalites at all.  The decision has been made to consider them fellow victims of the Sher'tul.#WHITE#"
	else
	...
	if self:isCompleted("shalore") then
		desc[#desc+1] = _t"#LIGHT_GREEN#* The Rhaloren have plunged Elvala into chaos.  It will be easy to wipe out the survivors.#WHITE#"
	else
	...
	if self:isCompleted("zigur") then
		desc[#desc+1] = _t"#LIGHT_GREEN#* The anti-mage headquarters is ruined, and their warriors scattered#WHITE#"
	else
	...
	if self:isCompleted("angolwen") then
		...
		desc[#desc+1] = _t"#SLATE#* The largest resistance is expected to come from the wizards of Angolwen.  Khulmanar will deal with them directly.#WHITE#"
	end
