...
newTalent{
	name = "Hollow", short_name = "REK_EVOLUTION_FALLEN_HOLLOW", image="talents/rek_evolution_doomed_hollow.png",
	type = {"uber/willpower", 1},
	uber = true,
	require = {
		stat = {wil = 50},
		level = 25,
		birth_descriptors={{"subclass", "Fallen"}},
		special={
			desc="10 points in Lomea talents",
			fct=function(self)
				return self:getTalentLevelRaw(self.T_FLN_CALL_SHADOWS) + self:getTalentLevelRaw(self.T_FLN_SHADOW_WARRIORS) + self:getTalentLevelRaw(self.T_FLN_SHADOW_MAGES) + self:getTalentLevelRaw(self.T_FLN_FOCUS_SHADOWS) >= 10
			end
		}
	},
	is_class_evolution = "Fallen",
	cant_steal = true,
	mode = "passive",
	is_spell = true,
	no_npc_use = true,
	...
	becomeHollow = function(self, t)
		...
		-- Unlearn old talents, learn new
		local lunacyLevel = self:getTalentLevelRaw(self.T_LUNACY)
		local removes = {
			["celestial/darkside"] = true,
			["celestial/combat"] = true,
		}
		t.unlearnTalents(self, t, removes)
		t.learnAndMaster(self, "undead/shadow-destruction", true, 1.3)
		t.learnAndMaster(self, "undead/shadow-magic", true, 1.3)
		self:learnTalent(self.T_LUNACY, true, lunacyLevel)
		self.unused_talents = self.unused_talents - lunacyLevel
...
#CRIMSON#This evolution fundamentally alters your class and character in a huge way. Do not take it lightly.#LAST#
You can no longer use infusions, you shrink by one size category, and your maximum life will be reduced by 5 life rating (this is retroactive).  You lose access to Celestial Combat and Darkside (except Lunacy), and all points you spent there will be refunded.
