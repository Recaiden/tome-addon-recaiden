...
function _M:displayTurntracker(scale, bx, by)
	...
	if game.player.changed and next(self.turntracker) then
	...
	local calcTime = function(act)
		...
		return {a=act, t=ticksRemaining}
	end
	if game.level then
		for uid, act in pairs(game.level.entities) do
			if game.party and game.party:hasMember(act) then
				entities[#entities+1] = calcTime(act)
			elseif player:hasLOS(act.x, act.y) and player:canSee(act) then
				entities[#entities+1] = calcTime(act)
			end
		end
	end
	...
	for i = 1, count do
		entities[i].a.turntracker_marker = i <= 26 and string.char(i+96) or "?"
	end

	-- extrapolate future turns assuming 1 action at normal speed per turn
	for i = 1, count do
	end
	...
	for i = 1, #entities do
		...
		if not self.turntracker[a] then
			...
			local text = ("#GOLD##{bold}#%s\n#WHITE##{normal}#Life: %d%%\nLevel: %d"):tformat(a:getName(), math.floor(100 * a.life / a.max_life), a.level)
			...
			self.turntracker[a] = {
				...
				function(x, y)
					...
					if not gtxt or self.turntracker[a].cur_turntracker_ticks ~= a.turntracker_ticks then
						local txt = tostring(math.ceil(a.turntracker_ticks))
						local fw, fh = self.buff_font_small:size(txt)
						self.turntracker[a].txt_turntracker_ticks = self.buff_font_small:draw(txt, fw, colors.WHITE.r, colors.WHITE.g, colors.WHITE.b, true)[1]
					...
					-- Disply letter mark over the portrait
		...
		if not game.mouse:updateZone("turntracker"..a.uid, bx+x*scale, by+y*scale, hs, hs, self.turntracker[a][4], scale) then
