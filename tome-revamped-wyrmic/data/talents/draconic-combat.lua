newTalent{
	...
	type = {"wild-gift/draconic-combat", 1}, require = techs_req1, points = 5,
...
newTalent{
	...
	type = {"wild-gift/draconic-combat", 2}, require = techs_req2, points = 5,
	equilibrium = 5,
	cooldown = 10,
	range = 0,
	speed = "weapon",
	...
	info = function(self, t)
		...
		return ([[You summon a powerful gust of wind, knocking back your foes within a radius of %d up to 3 tiles away and hitting them for %d%% weapon damage (including your shield, if you have one equipped).]]):format(self:getTalentRadius(t), damage*100)
	end,
}
...
newTalent{
	...
	action = function(self, t)
		...
		if source then damtype = source.status end 
		local x, y = self:getTarget(tg)
		if not x or not y then return nil end
		self:project(
			tg, x, y,
	...
	info = function(self, t)
		local source = self.rek_wyrmic_dragon_damage or
			{
				name="Physical",
				damtype=DamageType.REK_WYRMIC_NULL,
				nameStatus="unaffected otherwise (you don't have an aspect)",
				status=DamageType.REK_WYRMIC_NULL
			}
		local name = DamageType:get(source.damtype).text_color..DamageType:get(source.damtype).name.."#LAST#"
		local damtype = source.damtype
		local nameStatus = source.nameStatus
...
newTalent{
	...
	action = function(self, t)
		...
		game.level.map:particleEmitter(x, y, 1, "rek_wyrmic_dissolve", {})
	...
	info = function(self, t)
		...
		return ([[You strike the enemy with a rain of four fast blows each doing %d%% weapon damage as %s, with a 25%% chance that the enemy will be %s.  This will also attack with your shield, if you have one equipped.]]):format(100 * t.getDamage(self, t), name, nameStatus)
	end,
}
