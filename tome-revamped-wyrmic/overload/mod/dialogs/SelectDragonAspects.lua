...
function _M:init(actor, possibles)
   ...
   self.max_aspects = actor:callTalent(actor.T_REK_WYRMIC_MULTICOLOR, "getNumAspects")
   ...
   self.c_list = List.new{scrollbar=true, width=680, height=self.ih - 5 - self.c_ok.h, list=self.list, fct=function(item) self:use(item) end, select=function(item) self:select(item) end}
